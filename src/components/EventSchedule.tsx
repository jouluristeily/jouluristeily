...
export function EventSchedule({ events }: { events: EventData[] }) {
  ...
  const years = [...new Set(scheduledEvents.map(eventYear))].sort(
    (a, b) => b - a,
  );
  ...
  if (!years.length || selectedYear === undefined) {
    return (
      ...
      >
        ...
        <div className="event-schedule-empty">
          <p className="event-schedule-empty-message">
            Tapahtumaohjelma on parhaillaan valmistumassa. Tarkista myöhemmin
            saatavilla olevan käsiohjelman kautta.
          </p>
        </div>
      </section>
    );
            ...
            {section.events.map((event) => (
              ...
              >
                <div className="event-card-time">
                  <span className="event-card-hour">
                    {formatHour(event.startAt)}
                  </span>
                  <span className="event-card-day">
                    {formatDayAbbreviation(event.startAt)}
                  </span>
                </div>
