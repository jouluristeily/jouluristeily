...
export function PriceTables({
  ...
  if (!items.length) {
    return (
      <div className="prices-empty-state">
        <p className="prices-empty-message">
          Hinnastot ovat päivittymässä. Palaa myöhemmin tarkastamaan
          ajantasaiset hinnat.
        </p>
      </div>
    );
              ...
              {rows.map((item) => (
                ...
                >
                  ...
                  <div className="price-item-description-column">
                    {item.description ? (
                      <p className="price-item-description">
                        {item.description}
                      </p>
                    ) : null}
                  </div>
